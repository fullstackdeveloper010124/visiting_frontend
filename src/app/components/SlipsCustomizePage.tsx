...
interface SlipsCustomizePageProps {
  onMenuClick?: () => void;
}

const slipPresets = [
  { measurement: '8.5 x 3.5 Compliment Slip', boxes: '10', inStock: '5000', ordered: '500', balance: '4500', minQuantity: '500', costPerBox: '15.00' },
  { measurement: '8.27 x 3.9 Compliment Slip (DL)', boxes: '10', inStock: '5000', ordered: '500', balance: '4500', minQuantity: '500', costPerBox: '18.00' },
];

export function SlipsCustomizePage({ onMenuClick }: SlipsCustomizePageProps) {
  const [measurement, setMeasurement] = useState(slipPresets[0].measurement);
  const [boxes, setBoxes] = useState(slipPresets[0].boxes);
  const [inStock, setInStock] = useState(slipPresets[0].inStock);
  const [ordered, setOrdered] = useState(slipPresets[0].ordered);
  const [balance, setBalance] = useState(slipPresets[0].balance);
  const [minQuantity, setMinQuantity] = useState(slipPresets[0].minQuantity);
  const [costPerBox, setCostPerBox] = useState(slipPresets[0].costPerBox);
  ...
  useEffect(() => {
    const loadDraft = async () => {
      try {
        ...
        if (token) {
          const draftRes = await fetch('/api/v1/drafts/slip', {
          ...
          if (draftRes.ok && draftData.success && draftData.data) {
            ...
            if (draft.measurement) {
              ...
              const preset = slipPresets.find(p => p.measurement === draft.measurement);
              if (preset) {
            ...
            if (draft.uploadedSlip) setUploadedSlip(draft.uploadedSlip);
          }
        }
      } catch (err) {
        console.error('Failed to load slip draft:', err);
      } finally {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const response = await fetch('/api/v1/drafts/slip', {
          method: 'POST',
          headers: {
        ...
        console.error('Failed to auto-save slip draft:', err);
        setDraftStatus('idle');
      }
    }, 1500);
  ...
  const handleMeasurementSelection = (value: string) => {
    ...
    const preset = slipPresets.find(p => p.measurement === value);
    if (preset) {
      setBoxes(preset.boxes);
      setInStock(preset.inStock);
      setOrdered(preset.ordered);
      ...
      <main className="flex-1 overflow-hidden">
        <div className="flex h-full flex-col lg:flex-row">
          <div className="flex-1 w-full lg:max-w-md xl:max-w-lg border-r border-border bg-background overflow-y-auto p-4 md:p-6 shadow-sm z-10">
            <div className="mb-6">
              <h1 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-emerald-500 via-blue-500 to-violet-500">Compliment Slips Studio</h1>
              <p className="text-sm text-muted-foreground mt-1">Configure details and upload your design.</p>
            </div>
            <div className="space-y-6 pb-20">
              <div className="space-y-4">
                <h3 className="font-semibold text-emerald-600 uppercase tracking-wider text-xs border-b pb-2">Slip Configuration</h3>
                <div className="space-y-4">
                  <div>
                    <Label className="text-xs text-muted-foreground">Upload Slip Design</Label>
                    <input type="file" ref={slipInputRef} className="hidden" accept="image/*" onChange={(e) => { if (e.target.files?.[0]) setUploadedSlip(URL.createObjectURL(e.target.files[0])); }} />
                    <Button variant="outline" className="w-full justify-start mt-1 text-xs border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100" onClick={() => slipInputRef.current?.click()}>
                      <Upload className="mr-2 h-4 w-4" /> {uploadedSlip ? "Change Design" : "Upload Design"}
                    </Button>
                  </div>
                  <div className="grid grid-cols-2 gap-3 pt-2">
                    <div className="space-y-1.5 col-span-2">
                      <Label htmlFor="measurement">Slip Measurement</Label>
          ...
          <div className="flex-1 lg:flex-[1.5] bg-muted/30 p-4 md:p-8 overflow-y-auto relative flex flex-col items-center">
            ...
            <div className="w-full max-w-md md:max-w-xl space-y-10 flex flex-col items-center pb-20">
              <div className="w-full space-y-2">
                <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground ml-2">Slip View</span>
                <Card className="w-full aspect-[8.5/3.5] shadow-xl overflow-hidden rounded-sm border transition-all duration-300 relative bg-white flex items-center justify-center p-8 bg-no-repeat bg-center" style={{ backgroundImage: uploadedSlip ? `url(${uploadedSlip})` : 'none', backgroundSize: 'cover' }}>
                  {!uploadedSlip && <div className="text-center text-muted-foreground"><Upload className="mx-auto h-8 w-8 mb-2 opacity-50" /><p>Upload a design to preview your slip</p></div>}
                </Card>
              </div>
            </div>
          </div>
          ...
          <Card className="w-full max-w-md bg-background border border-border shadow-2xl overflow-hidden rounded-xl animate-in zoom-in-95 duration-200">
            ...
            <CardHeader className="text-center pt-8">
              ...
              <CardDescription className="text-sm text-muted-foreground mt-1">
                We have generated an approval request for your compliment slip design.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4 px-6 pb-6">
              ...
              {uploadedSlip && (
                <div className="space-y-1.5 text-center my-3">
                  ...
                  <div className="mx-auto max-w-[160px] aspect-[8.5/3.5] rounded border border-border shadow-sm bg-white bg-no-repeat bg-center"
                       style={{ 
                         backgroundImage: `url(${uploadedSlip})`,
